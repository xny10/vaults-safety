...
/**
 * Persistent reply keyboard (the buttons that sit above the message box).
 * Each button sends its label as plain text, which the message handler
 * maps to the matching action.
 */
export function persistentMenu() {
  return {
    keyboard: [
      [{ text: "📁 Website" }, { text: "➕ Add Account" }],
      [{ text: "🔍 Search" }, { text: "📊 Stats" }],
      [{ text: "🔄 Sync" }, { text: "🏠 Menu" }],
    ],
    resize_keyboard: true,
    is_persistent: true,
    input_field_placeholder: "Pilih menu atau ketik...",
  };
}
...
export function accountDetail(accountId, websiteId) {
  return {
    inline_keyboard: [
      [
        btn("✏️ Edit", `account:edit:${accountId}`),
