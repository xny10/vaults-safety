...
async function handleAccountCallback(cq, parts, ack) {
  ...
  if (action === "showpass") {
    ...
    // Reveal inline as <code> so it can be tap-to-copy in Telegram.
    return editMessageText(
      chatId,
      messageId,
      render.accountDetailText(account, password),
      kb.accountDetail(accountId, account.websiteId, true)
    );
  }
  ...
  if (action === "hidepass") {
    ...
    return editMessageText(
      ...
      messageId,
      render.accountDetailText(account, null),
      kb.accountDetail(accountId, account.websiteId, false)
    );
  }
