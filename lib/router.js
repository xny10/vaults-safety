...
async function handleMessage(message) {
  ...
  if (text === "/start" || text === "/menu" || text === "🏠 Menu") {
    ...
    // Show the persistent reply keyboard once, plus the inline main menu.
    await sendMessage(chatId, "Panel siap. Gunakan tombol di bawah 👇", kb.persistentMenu());
  ...
  // Reply-keyboard buttons arrive as plain text. Map them to actions.
  if (await handleMenuButton(chatId, userId, text)) return;
...
/**
 * Handle taps on the persistent reply keyboard (sent as plain text).
 * Returns true if the text was a known menu button.
 */
async function handleMenuButton(chatId, userId, text) {
  switch (text) {
    case "📁 Website": {
    ...
    case "➕ Add Account": {
      ...
      await sendMessage(
        chatId,
        "Pilih website:",
        kb.selectWebsiteForAccount(websites)
      );
      return true;
    }
    case "🔍 Search": {
      ...
      await sendMessage(chatId, "Kirim email / keyword website:");
      return true;
    }
    case "📊 Stats": {
      const stats = await getStats(userId);
      await sendMessage(chatId, render.statsText(stats), kb.backToMenu());
      return true;
    }
    case "🔄 Sync": {
      const result = await runSync(userId);
      await sendMessage(chatId, render.syncText(result), kb.backToMenu());
      return true;
    }
    default:
      return false;
...
async function handleAccountCallback(cq, parts, ack) {
  ...
  if (action === "view") {
    ...
    // Single-user panel: always reveal the password (tap-to-copy <code>).
    const password = await showPassword(userId, accountId);
    return editMessageText(
      chatId,
      messageId,
      render.accountDetailText(account, password),
